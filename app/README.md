...
│   ├── database.hpp/cpp     # SQLite storage: submissions + questions (CRUD)
...
├── public/
│   ├── index.html           # Role-based redirect entry point
│   ├── login.html / signup.html / admin-login.html
│   ├── auth.js / auth.css   # Client-side auth (demo: localStorage)
│   ├── compiler.html        # CodeMirror editor UI
│   ├── compiler.css / script.js
│   ├── admin_dashboard.html # Admin panel (questions & test cases)
│   ├── admin.js / admin.css # Admin logic (questions wired to /api/questions)
│   └── styles.css
├── CMakeLists.txt           # Build system (auto-fetches Crow, Asio, SQLite)
├── Dockerfile               # Multi-stage build
├── docker-compose.yml       # Container orchestration + persistent DB volume
├── .dockerignore
├── .env.example
└── README.md
```

> Documentation (design report, specs, screenshots) lives in the top-level `../docs/` folder.
